...
You can pass your custom hashing algorithm as the second argument for the hook. Signature:<br />
`(key: any) => number`

<Playground>
  {() => {
    const hashingAlgorithm = useRef((key) => key.split('').reduce((curr, next) => curr + next.charCodeAt(0), 0))
    ...
    const hashTable = useHashTable({}, hashingAlgorithm.current)
